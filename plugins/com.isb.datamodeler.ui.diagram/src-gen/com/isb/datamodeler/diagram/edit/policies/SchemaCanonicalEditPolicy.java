...
public class SchemaCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected void refreshSemantic() {
		...
		for (View v : getViewChildren()) {
			if (isShortcut(v)) {
				if (DatamodelerDiagramUpdater.isShortcutOrphaned(v)) {
					orphaned.add(v);
				}
				continue;
			}
	...
	protected boolean addListenerFilter(String filterId,
		...
		if (element instanceof ESchema) {
			for (EObject child : element.eContainer().eContents())
				if (child instanceof ESchema && child != element)
					super.addListenerFilter(filterId, listener, child);
		}
