 ...
 * @since 2.1
 */
public class DataModelerWorkingSetFilterActionGroup extends ActionGroup {
	 ...
	 */
    public DataModelerWorkingSetFilterActionGroup(Shell shell,
        ...
        mruList = new CompoundContributionItem() {
			...
			protected IContributionItem[] getContributionItems() {
				...
				List<IContributionItem> items = new ArrayList<IContributionItem>(workingSets.length);
				List<IWorkingSet> sortedWorkingSets = Arrays.asList(workingSets);
				Collections.sort(sortedWorkingSets, new DataModelerWorkingSetComparator());

				int mruMenuCount = 0;
				for (Iterator<IWorkingSet> i = sortedWorkingSets.iterator(); i.hasNext();) {
					IWorkingSet workingSet = (IWorkingSet) i.next();
					if (workingSet != null && workingSet.getId().equals(DataModelerWorkingSetPage.DATAMODELER_WORKING_SET_ID)) {
						IContributionItem item = new WorkingSetMenuContributionItem(
								++mruMenuCount,
								DataModelerWorkingSetFilterActionGroup.this, workingSet);
						items.add(item);
					}
				}
        ...
        workbenchWindow = getWorkbenchWindowForShell(shell);
	...
	private void firePropertyChange(IWorkingSet newWorkingSet, IWorkingSet oldWorkingSet) {
        ...
        if (workingSetUpdater != null) {
            workingSetUpdater.propertyChange(new PropertyChangeEvent(this,
                    DataModelerWorkingSetFilterActionGroup.CHANGE_WORKING_SET,
